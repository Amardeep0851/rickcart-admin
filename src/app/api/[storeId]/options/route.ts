...
import { createOptionsWithValues, fetchOptionWithName } from "@/lib/services/options/options-services";
...
export async function POST(req:Request,{params}:{params:Promise<{storeId:string}>}){
  try {
    ...
    if(values.length === 0){
      return new NextResponse("At least one value is required.", {status:400})
    }
